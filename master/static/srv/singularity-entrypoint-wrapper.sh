...
# HACK: Probably a better/smarter way to do this.
# Overlay the contents of /determined-archives onto our files. Can't really do this with bind
# mounting directly since you can't know what will exist in the container, and bind mounting an
# existing dir shadows it.
# Create clone of any directories under /dispatcher for this process and setup links
if [ -d $ROOT ]; then
    for dir in $ROOT/*; do
        if [[ -d $dir ]]; then
            log_debug "INFO: Clone $dir -> /"
            cp -p -n -R $dir / >&2
        fi
    done
fi
